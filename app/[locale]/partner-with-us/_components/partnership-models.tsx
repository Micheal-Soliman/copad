...
function PartnershipModelRow({ block, index, locale }: { block: ContentBlock; index: number; locale: Locale }) {
  ...
  return <motion.article id={`partnership-${index + 1}`} ref={rowRef} className={`relative grid min-h-[42rem] scroll-mt-40 overflow-hidden rounded-[2.25rem] border lg:min-h-[35rem] lg:grid-cols-12 ${dark ? "border-white/10 bg-copad-deep text-white" : "border-copad-deep/10 bg-white text-copad-deep"}`}>
    ...
    <motion.div style={reduceMotion ? undefined : { y: copyY, opacity: copyOpacity }} className={`relative flex flex-col justify-between p-7 sm:p-10 lg:col-span-5 lg:p-12 ${index % 2 ? "lg:order-1" : ""}`}>
      ...
      <div className="relative">
        <p className="text-[8px] font-black uppercase tracking-[.22em] text-copad-green">{ui.model} · 0{index + 1}</p>
  ...
  return <section id="models" dir={ar ? "rtl" : "ltr"} className="relative overflow-clip bg-copad-sand px-4 py-20 sm:px-8 sm:py-28 lg:px-12 lg:py-32">
    ...
    <div className="relative mx-auto max-w-[1440px]">
      <motion.header whileInView={reduceMotion ? undefined : { y: [14, 0] }} viewport={{ once: true, amount: .35 }} transition={{ duration: .65, ease }} className="grid gap-7 border-b border-copad-deep/12 pb-9 lg:grid-cols-[.9fr_1.1fr] lg:items-end">
        <div><p className="text-[9px] font-black uppercase tracking-[.22em] text-copad-green">{ui.areas}</p><h2 className="mt-4 max-w-[19ch] font-display text-[2.4rem] leading-[1.05] tracking-[-.04em] text-copad-deep sm:text-[clamp(3rem,4.5vw,4.8rem)]">{ui.modelsTitle.map(line => <span className="block" key={line}>{line}</span>)}</h2></div>
        <p className="max-w-2xl text-sm leading-7 text-copad-deep/60 sm:text-base sm:leading-8 lg:justify-self-end">{ui.modelsBody}</p>
      </motion.header>

      <nav aria-label={ui.jump} className="sticky top-20 z-30 -mx-1 mt-5 flex gap-2 overflow-x-auto rounded-full border border-copad-deep/10 bg-white/78 p-1.5 shadow-[0_14px_35px_rgba(6,79,120,.08)] [scrollbar-width:none] backdrop-blur-xl [&::-webkit-scrollbar]:hidden">
      ...
      <div className="mt-8 space-y-6 lg:mt-10 lg:space-y-8">
        {blocks.map((block, index) => <PartnershipModelRow key={block.title} block={block} index={index} locale={locale} />)}
      </div>
    </div>
  </section>;
}
