...
import { getPartnershipData, partnershipSlugs, type PartnershipSlug } from "@/content/partnerships";
import { getUiCopy } from "@/content/ui";

const ease = [0.22, 1, 0.36, 1] as const;

function RouteStep({ number, label, text, caption }: { number: string; label: string; text: string; caption: string }) {
  ...
  return <motion.div ref={ref} style={reduceMotion ? undefined : { y, scale }} className="relative overflow-hidden rounded-[2rem] border border-white/12 bg-white/[.055] p-7 backdrop-blur-sm sm:p-9">
    ...
    <p className="mt-4 text-[9px] font-black uppercase tracking-[.16em] text-white/38">{caption}</p>
  </motion.div>;
}
    ...
    <section ref={heroRef} className="relative overflow-hidden bg-copad-deep px-4 pb-5 pt-24 text-white sm:px-8 lg:px-12 lg:pb-8">
      ...
      <div dir={ar ? "rtl" : "ltr"} className="relative mx-auto grid min-h-[calc(100svh-7rem)] max-w-[1440px] overflow-hidden rounded-[2.25rem] border border-white/12 lg:grid-cols-2">
        <motion.div style={reduceMotion ? undefined : { y: copyY, opacity: copyOpacity }} className="relative order-2 flex flex-col justify-end p-7 sm:p-10 lg:order-1 lg:p-14">
          ...
          <Link href={`/${locale}/partner-with-us#models`} className="mb-auto inline-flex w-fit items-center gap-2 pt-2 text-[8px] font-black uppercase tracking-[.18em] text-white/52 transition hover:text-white"><span aria-hidden="true" className="rtl:rotate-180">←</span>{ui.back}</Link>
    ...
    <section id="pathway" ref={pathwayRef} dir={ar ? "rtl" : "ltr"} className="relative bg-copad-deep px-4 py-20 text-white sm:px-8 sm:py-28 lg:px-12 lg:py-32">
      ...
      <div className="relative mx-auto grid max-w-[1320px] gap-12 lg:grid-cols-[.72fr_1.28fr] lg:gap-20">
        <div className="self-start lg:sticky lg:top-28">
          <p className="text-[9px] font-black uppercase tracking-[.23em] text-copad-green">{ui.pathway}</p>
          <h2 className="mt-5 max-w-[17ch] font-display text-[clamp(2.8rem,4vw,4.6rem)] leading-[1.02] tracking-[-.045em]">{ui.pathwayTitle}</h2>
          <p className="mt-7 max-w-md text-sm leading-7 text-white/58 sm:text-base sm:leading-8">{ui.pathwayBody}</p>
        </div>
        <div className="space-y-5">{data.highlights.map((item, index) => <RouteStep key={item} number={`0${index + 1}`} label={stepLabels[index]} text={item} caption={ui.fromAlignment} />)}</div>
      </div>
    </section>
