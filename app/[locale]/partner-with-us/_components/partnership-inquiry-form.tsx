...
import { submitPartnershipInquiryAction, type SubmissionState } from "@/app/actions/submissions";
import type { PartnershipSlug } from "@/content/partnerships";
import { getPartnershipData } from "@/content/partnerships";
...
export function PartnershipInquiryForm({ locale, slug }: { locale: Locale; slug: PartnershipSlug }) {
  const [state, action, pending] = useActionState(submitPartnershipInquiryAction, { status: "idle" } satisfies SubmissionState);
  const data = getPartnershipData(locale, slug);
  const ui = getUiCopy(locale).partners;
  const reduceMotion = useReducedMotion();

  return <form action={action} className="grid gap-4 rounded-[1.75rem] border border-copad-deep/10 bg-white p-5 shadow-[0_28px_80px_rgba(6,79,120,.10)] sm:grid-cols-2 sm:p-8">
    <input type="hidden" name="locale" value={locale} />
    <input type="hidden" name="partnershipSlug" value={slug} />
    {data.fields.map((field, index) => <motion.label key={field.name} initial={reduceMotion ? false : { y: 14 }} whileInView={{ y: 0 }} viewport={{ once: true, amount: .3 }} transition={{ duration: .48, delay: Math.min(index * .045, .22), ease: [0.22, 1, 0.36, 1] }} className={`text-xs font-black text-copad-deep/72 ${(field.type === "textarea" || field.type === "file") ? "sm:col-span-2" : ""}`}>
      ...
      {field.type === "select" ? <select className={fieldClass} name={field.name} required={field.required} defaultValue=""><option value="" disabled>{ui.select}</option>{field.options?.map((option) => <option key={option}>{option}</option>)}</select>
    ...
    <motion.div initial={reduceMotion ? false : { y: 14 }} whileInView={{ y: 0 }} viewport={{ once: true, amount: .35 }} transition={{ duration: .5, delay: .16 }} className="mt-2 flex flex-col items-start gap-3 sm:col-span-2 sm:flex-row sm:items-center">
      <button disabled={pending} type="submit" className="min-h-12 rounded-full bg-copad-green px-7 text-xs font-black text-white shadow-[0_15px_34px_rgba(0,163,196,.22)] transition duration-300 hover:-translate-y-0.5 hover:bg-copad-deep disabled:opacity-55">{pending ? ui.sending : ui.submit}</button>
      <p aria-live="polite" className={`text-xs font-bold ${state.status === "error" ? "text-copad-red" : "text-copad-green"}`}>{state.status === "success" ? ui.received : state.status === "error" ? ui.error : ""}</p>
    </motion.div>
  </form>;
}
