...
export function PartnershipInquiryForm({ locale, slug }: { locale: Locale; slug: PartnershipSlug }) {
  ...
  return <form onSubmit={(event) => { event.preventDefault(); setSubmitted(true); }} className="grid gap-4 rounded-[1.75rem] border border-copad-deep/10 bg-white p-5 shadow-[0_28px_80px_rgba(6,79,120,.10)] sm:grid-cols-2 sm:p-8">
    ...
    <motion.div initial={reduceMotion ? false : { y: 14 }} whileInView={{ y: 0 }} viewport={{ once: true, amount: .35 }} transition={{ duration: .5, delay: .16 }} className="mt-2 flex flex-col items-start gap-3 sm:col-span-2 sm:flex-row sm:items-center">
      <button type="submit" className="min-h-12 rounded-full bg-copad-green px-7 text-xs font-black text-white shadow-[0_15px_34px_rgba(0,163,196,.22)] transition duration-300 hover:-translate-y-0.5 hover:bg-copad-deep">{ui.submit}</button>
      <p aria-live="polite" className={`text-xs font-bold text-copad-green transition ${submitted ? "opacity-100" : "opacity-0"}`}>{ui.received}</p>
    </motion.div>
  </form>;
}
