...
export function AboutJourneyLine() {
  ...
  return (
    <motion.div aria-hidden="true" className="pointer-events-none fixed top-[27%] end-5 z-[62] hidden h-[46%] w-px bg-copad-deep/10 lg:block" style={{ opacity }}>
      <motion.span className="absolute inset-0 origin-top bg-linear-to-b from-copad-green via-copad-green to-copad-deep/45 shadow-[0_0_16px_rgba(0,163,196,.28)]" style={{ scaleY: progress }} />
      ...
      <motion.span className="absolute start-1/2 size-3 -translate-x-1/2 -translate-y-1/2 rounded-full bg-copad-green shadow-[0_0_18px_5px_rgba(0,163,196,.42)]" style={{ top: useTransform(progress, [0, 1], ["0%", "100%"]) }} />
    </motion.div>
  );
}
