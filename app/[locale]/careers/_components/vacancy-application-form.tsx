...
export function VacancyApplicationForm({ locale, vacancy }: { locale: Locale; vacancy: LocalizedCareerVacancy }) {
  ...
  if (submitted) return <div role="status" className="rounded-[1.75rem] bg-copad-white p-8 text-copad-deep shadow-[0_30px_80px_rgba(1,61,96,.12)] sm:p-10">
  ...
  return <form onSubmit={(event) => { event.preventDefault(); setSubmitted(true); }} className="grid gap-5 rounded-[1.75rem] bg-copad-white p-5 text-copad-deep shadow-[0_30px_80px_rgba(1,61,96,.12)] sm:grid-cols-2 sm:p-8 lg:p-10">
    ...
    <button type="submit" className="group col-span-full inline-flex min-h-12 items-center justify-center gap-4 rounded-full bg-copad-green px-7 text-xs font-black text-white shadow-[0_16px_36px_rgba(0,144,175,.22)] transition hover:-translate-y-0.5 hover:bg-copad-deep sm:col-span-1">{labels.submit}<ArrowRightIcon className="transition-transform group-hover:translate-x-1 rtl:rotate-180 rtl:group-hover:-translate-x-1" /></button>
  </form>;
}
