...
export function CareersHero({ locale, content }: { locale: Locale; content: Section }) {
  const ref = useRef<HTMLElement>(null); const reducedMotion = useReducedMotion(); const isArabic = locale === "ar"; const ui = getUiCopy(locale).careers;
  ...
  return <section ref={ref} id="home" style={homeScrollSceneStyle(2)} className="relative h-[var(--scroll-scene-height)] bg-copad-deep text-white">
    <div className="sticky top-0 h-[100svh] overflow-hidden">
      <motion.div className="absolute inset-0" style={reducedMotion ? undefined : { scale: imageScale, x: imageX }}><Image src="/images/brand/careers-lab-coat.jpeg" alt={isArabic ? "معطف مختبر يحمل هوية كوباد" : "Branded Copad laboratory coat"} fill priority className="object-cover object-center" sizes="100vw" /></motion.div>
      <div className="absolute inset-0 bg-[linear-gradient(90deg,rgba(6,79,120,.96)_0%,rgba(6,79,120,.78)_35%,rgba(6,79,120,.08)_76%)] rtl:bg-[linear-gradient(270deg,rgba(6,79,120,.96)_0%,rgba(6,79,120,.78)_35%,rgba(6,79,120,.08)_76%)]" />
