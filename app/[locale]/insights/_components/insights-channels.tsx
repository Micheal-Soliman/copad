...
export function InsightsChannels({ locale, blocks }: { locale: Locale; blocks: ContentBlock[] }) {
  ...
  return <section ref={ref} id="channels" style={scrollSceneStyle(blocks.length)} className="relative h-[var(--scroll-scene-height)] bg-copad-sand"><div className="sticky top-0 h-[100svh] overflow-hidden">
    <div className="absolute inset-0 bg-[linear-gradient(90deg,rgba(15,61,57,.025)_1px,transparent_1px)] bg-[size:24vw_100%]" />
    <div dir={ar?"rtl":"ltr"} className="relative z-10 mx-auto flex h-full max-w-[1440px] flex-col px-4 pt-20 pb-20 sm:px-8 lg:px-12 lg:pt-24">
      <header className="flex shrink-0 items-end justify-between gap-4 border-b border-copad-deep/12 pb-4"><div><p className="text-[8px] font-black tracking-[.2em] text-copad-green uppercase">{ar?"فهرس العدد":"Issue index"}</p><h2 className={`${ar?"font-sans font-black":"font-display"} mt-2 text-3xl tracking-[-.04em] sm:text-5xl`}>{ar?"اختر مسار القراءة":"Choose your reading stream"}</h2></div><span dir="ltr" className="font-display text-4xl">0{active+1}<small className="text-sm text-copad-deep/25"> / 0{blocks.length}</small></span></header>
...
function JournalSheet({block,index,active,ar,reduce}:{block:ContentBlock;index:number;active:number;ar:boolean;reduce:boolean}) {
  ...
  return <motion.article initial={false} animate={reduce?undefined:{y:passed?-150:distance*18,scale:current?1:Math.max(.92,1-Math.abs(distance)*.03),rotateX:current?0:distance>0?-2:3,rotateZ:current?0:(ar?-1:1)*distance*.4,opacity:passed?0:1,zIndex:20-distance}} transition={{duration:scrollSystem.scene.transitionDuration,ease}} className={`absolute inset-x-0 top-1/2 mx-auto grid h-[min(65svh,34rem)] max-w-6xl -translate-y-1/2 overflow-hidden rounded-[.45rem_2rem_2rem_.45rem] border bg-copad-white text-copad-deep shadow-[0_35px_95px_rgba(15,61,57,.16)] [transform-style:preserve-3d] sm:h-[min(60svh,32rem)] lg:h-[min(55svh,31rem)] rtl:rounded-[2rem_.45rem_.45rem_2rem] ${current?"border-copad-green/35":"border-copad-deep/10"}`} style={{zIndex:20-distance}}>
    <div className="grid h-full lg:grid-cols-[.82fr_1.18fr]">
      <div className="relative overflow-hidden border-e border-copad-deep/10 p-5 sm:p-7 lg:p-9"><div className="flex justify-between border-b border-copad-deep/12 pb-2 text-[7px] font-black tracking-[.18em] text-copad-green uppercase"><span>COPAD MEDICAL JOURNAL</span><span>0{index+1}</span></div><p className="mt-4 text-[8px] font-black tracking-[.18em] text-copad-green uppercase sm:mt-5">{ar?"القسم التحريري":"Editorial section"}</p><h3 className={`${ar?"font-sans font-black leading-[1.08]":"font-display leading-[.92]"} mt-3 text-[clamp(1.8rem,5vw,4.3rem)] tracking-[-.055em]`}>{block.title}</h3><p className="mt-3 max-w-xl text-[11px] leading-5 text-copad-deep/62 sm:text-sm sm:leading-6">{block.body}</p><span className="absolute -end-7 -bottom-16 font-display text-[15rem] leading-none text-copad-deep/[.035]">0{index+1}</span></div>
      <div className="flex flex-col justify-center p-5 sm:p-7 lg:p-9"><p className="mb-1 text-[8px] font-black tracking-[.18em] text-copad-green uppercase sm:mb-2">{ar?"موضوعات مقترحة":"Featured reading"}</p>{block.items?.map((item,i)=><motion.div key={item} animate={current?{opacity:1,x:0}:{opacity:.35,x:ar?-12:12}} transition={{duration:scrollSystem.scene.transitionDuration,delay:current?.24+i*.1:0,ease}} className="group flex items-baseline gap-4 border-b border-copad-deep/10 py-2.5 last:border-0 sm:py-3"><span className="font-display text-lg text-copad-green">{String(i+1).padStart(2,"0")}</span><span className="text-xs font-bold leading-5 text-copad-deep/68 transition group-hover:text-copad-deep sm:text-sm">{item}</span></motion.div>)}</div>
    </div>
    <span className="absolute end-0 top-0 size-16 bg-[linear-gradient(225deg,#eeebe5_0_49%,rgba(15,61,57,.12)_50%,transparent_52%)] shadow-[-8px_8px_18px_rgba(15,61,57,.08)]" />
  </motion.article>;
}
