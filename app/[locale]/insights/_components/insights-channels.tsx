  ...
  return <section ref={ref} id="channels" style={homeScrollSceneStyle(blocks.length)} className="relative h-[var(--scroll-scene-height)] bg-copad-sand"><div className="sticky top-0 h-[100svh] overflow-hidden">
    ...
    <div dir={ar?"rtl":"ltr"} className="relative z-10 mx-auto flex h-full max-w-[1440px] flex-col px-4 pt-20 pb-20 sm:px-8 lg:px-12 lg:pt-24">
      ...
      <div className="relative min-h-0 flex-1 [perspective:1800px]">
        {blocks.map((block,index)=><JournalSheet key={block.title} block={block} index={index} active={active} ar={ar} reduce={!!reduce} editorial={ui.editorial} featured={ui.featured}/>) }
      </div>
...
function JournalSheet({block,index,active,ar,reduce,editorial,featured}:{block:ContentBlock;index:number;active:number;ar:boolean;reduce:boolean;editorial:string;featured:string}) {
  ...
  return <motion.article initial={false} animate={reduce?undefined:{y:passed?-120:distance*16,scale:current?1:Math.max(.94,1-Math.abs(distance)*.025),rotateX:current?0:distance>0?-1.25:2,rotateZ:current?0:(ar?-1:1)*distance*.25,opacity:passed?0:1,zIndex:20-distance}} transition={{duration:.76,ease}} className={`absolute inset-x-0 top-1/2 mx-auto grid h-[min(65svh,34rem)] max-w-6xl -translate-y-1/2 overflow-hidden rounded-[.45rem_2rem_2rem_.45rem] border bg-copad-white text-copad-deep shadow-[0_35px_95px_rgba(6,79,120,.16)] [transform-style:preserve-3d] sm:h-[min(60svh,32rem)] lg:h-[min(55svh,31rem)] rtl:rounded-[2rem_.45rem_.45rem_2rem] ${current?"border-copad-green/35":"border-copad-deep/10"}`} style={{zIndex:20-distance}}>
    <div className="grid h-full lg:grid-cols-[.82fr_1.18fr]">
      <div className="relative overflow-hidden border-e border-copad-deep/10 p-5 sm:p-7 lg:p-9"><div className="flex justify-between border-b border-copad-deep/12 pb-2 text-[7px] font-black tracking-[.18em] text-copad-green uppercase"><span>COPAD MEDICAL JOURNAL</span><span>0{index+1}</span></div><p className="mt-4 text-[8px] font-black tracking-[.18em] text-copad-green uppercase sm:mt-5">{editorial}</p><h3 className="mt-3 max-w-[13ch] font-display text-[clamp(1.8rem,5vw,4rem)] leading-[1.02] tracking-[-.045em]">{block.title}</h3><p className="mt-4 max-w-xl text-[11px] leading-5 text-copad-deep/62 sm:text-sm sm:leading-6">{block.body}</p><span className="absolute -end-7 -bottom-16 font-display text-[15rem] leading-none text-copad-deep/[.035]">0{index+1}</span></div>
