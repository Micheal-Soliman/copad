...
export function TherapyAreasHero({ locale, title, intro, areas }: TherapyAreasHeroProps) {
  ...
  const folioScale = useTransform(scrollYProgress, [0, 0.42, 1], [0.97, 1, 1.03]);
  const folioY = useTransform(scrollYProgress, [0, 0.48, 1], [18, 0, -16]);
  const photoScale = useTransform(scrollYProgress, [0, 0.45, 1], [1.12, 1.035, 1]);
      ...
      <div className="relative isolate min-h-[100svh] overflow-hidden bg-copad-deep text-white lg:sticky lg:top-0 lg:h-screen">
        ...
        <div dir={isArabic ? "rtl" : "ltr"} className="mx-auto grid min-h-[100svh] max-w-[1440px] items-center gap-9 px-4 pt-24 pb-8 sm:px-8 sm:pt-28 lg:h-screen lg:min-h-0 lg:grid-cols-[.82fr_1.18fr] lg:gap-12 lg:px-12 lg:pt-[5.75rem] lg:pb-4">
          <motion.div className="relative z-20" style={scrollDriven ? { y: copyY } : undefined}>
            ...
            <motion.h1 initial={reduceMotion ? false : { opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: .78, delay: .08, ease }} className={`${isArabic ? "mt-5 max-w-3xl font-sans text-[clamp(3.4rem,14vw,5.6rem)] leading-[1.1] font-black tracking-[-.035em] lg:mt-4 lg:text-[clamp(4.5rem,6.5vw,6.8rem)]" : "mt-5 max-w-3xl text-pretty font-display text-[clamp(4rem,15vw,6.2rem)] leading-[1.04] tracking-[-.045em] lg:mt-4 lg:text-[clamp(4.8rem,6.2vw,6.5rem)]"}`}>
              {title}
            </motion.h1>

            <motion.div className="mt-7 h-px max-w-lg origin-start bg-linear-to-r from-copad-green via-copad-green/35 to-transparent lg:mt-5" initial={reduceMotion ? false : { scaleX: 0 }} animate={{ scaleX: 1 }} transition={{ duration: .9, delay: .35, ease }} />
...
function TherapyIndexItem({ area, index, progress, scrollDriven, reduceMotion }: { area: string; index: number; progress: MotionValue<number>; scrollDriven: boolean; reduceMotion: boolean }) {
  const opacity = useTransform(progress, [.1, .27, 1], [.18, 1, 1]);
  const y = useTransform(progress, [.1, .27, 1], [8, 0, 0]);
  const lineScale = useTransform(progress, [.1, .28], [0, 1]);

  return (
    <motion.li
      initial={!scrollDriven && !reduceMotion ? { opacity: .35, y: 8 } : false}
      animate={!scrollDriven ? { opacity: 1, y: 0 } : undefined}
      transition={{ duration: .55, delay: .42 + index * .045, ease }}
      style={scrollDriven ? { opacity, y } : undefined}
      className="relative min-h-14 border-t border-copad-deep/10 py-2.5 sm:min-h-16 sm:py-3"
    >
      <motion.span
        aria-hidden="true"
        className="absolute inset-x-0 top-0 h-px origin-start bg-copad-green rtl:origin-right"
        style={scrollDriven ? { scaleX: lineScale } : undefined}
        initial={!scrollDriven && !reduceMotion ? { scaleX: 0 } : false}
        animate={!scrollDriven ? { scaleX: 1 } : undefined}
        transition={{ duration: .55, delay: .46 + index * .045, ease }}
      />
