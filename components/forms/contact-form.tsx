...
export function ContactForm({ locale, variant = "default", initialCategory = "" }: ContactFormProps) {
  ...
  return (
    <form className={`grid gap-4 ${isDispatch ? "grid-cols-1 sm:grid-cols-2" : "rounded-[1.5rem] p-5 sm:rounded-[2rem] sm:p-8"} ${variant === "immersive" ? "border border-copad-deep/8 bg-white/88 shadow-[inset_0_1px_0_rgba(255,255,255,.9)]" : isDispatch ? "bg-transparent" : "bg-white shadow-[0_24px_70px_rgba(6,79,120,.1)]"}`}>
      ...
      <button className={`group relative isolate min-h-12 w-full overflow-hidden rounded-full bg-copad-green px-7 py-3 text-xs font-black text-white shadow-[0_15px_34px_rgba(0,163,196,.2)] transition duration-500 hover:-translate-y-1 hover:shadow-[0_22px_44px_rgba(6,79,120,.2)] sm:w-auto sm:justify-self-start ${isDispatch ? "sm:col-span-2" : ""}`} type="button"><span aria-hidden="true" className="absolute inset-0 -z-10 origin-bottom scale-y-0 bg-copad-deep transition-transform duration-500 group-hover:scale-y-100" />{copy.utility.submit}</button>
    </form>
  );
}
