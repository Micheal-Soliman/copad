...
const fieldClass = "mt-2 w-full rounded-2xl border border-copad-deep/12 bg-white px-4 py-3 text-sm text-copad-deep outline-none transition placeholder:text-copad-deep/35 focus:border-copad-green focus:ring-4 focus:ring-copad-green/10";

export function ContactForm({ locale }: { locale: Locale }) {
  const copy = siteCopy[locale];
  const [name, email, category, message] = copy.utility.formLabels;
  return (
    <form className="grid gap-5 rounded-[2rem] bg-white p-6 shadow-[0_24px_70px_rgba(15,61,57,.1)] sm:p-8">
      ...
      <button className="justify-self-start rounded-full bg-copad-green px-6 py-3 text-xs font-black text-white transition hover:-translate-y-0.5 hover:bg-copad-deep" type="button">{copy.utility.submit}</button>
    </form>
  );
}
