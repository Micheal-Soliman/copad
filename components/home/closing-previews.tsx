      ...
      <div>
        <motion.div initial={reduceMotion ? false : { opacity: 0, y: 42, scale: 0.985 }} whileInView={{ opacity: 1, y: 0, scale: 1 }} viewport={{ once: true, amount: 0.2 }} transition={{ duration: 0.9, ease }} className="relative z-10 mx-auto flex w-full max-w-[1440px] flex-col overflow-hidden rounded-[2rem] bg-copad-deep shadow-[0_26px_70px_rgba(1,61,96,.14)] sm:rounded-[3rem] sm:shadow-[0_34px_100px_rgba(1,61,96,.16)] lg:h-[clamp(32rem,72vh,38rem)] lg:flex-row">
          ...
          {cards.map((card, index) => {
            ...
            return (
              <motion.article key={card.href} animate={reduceMotion ? undefined : { flexGrow: isActive ? 1.2 : 0.8 }} transition={{ duration: 0.9, ease }} onMouseEnter={() => setActive(index)} onFocusCapture={() => setActive(index)} onTap={() => setActive(index)} tabIndex={0} className={`group relative overflow-hidden px-6 pt-6 pb-10 transition-[height] duration-[850ms] ease-[cubic-bezier(.22,1,.36,1)] sm:px-10 sm:pt-10 sm:pb-12 lg:h-auto lg:min-h-0 lg:basis-0 lg:px-10 lg:pt-10 lg:pb-14 ${isActive ? "h-[31rem]" : "h-44"} ${dark ? "bg-copad-deep text-white" : "bg-copad-white text-copad-deep"}`}>
