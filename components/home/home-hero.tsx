...
export function HomeHero(props: HomeHeroProps) {
  ...
  return (
    <section className="relative flex min-h-svh items-center overflow-hidden bg-copad-deep px-5 pt-24 pb-10 text-center text-white sm:px-8 lg:px-12">
      <Image
      ...
      >
        ...
        <motion.h1 variants={line} custom={1 * direction} className="mt-4 will-change-transform font-display text-5xl leading-[.94] tracking-[-0.055em] text-balance sm:text-7xl lg:text-[6.75rem]">
          {props.title}
        </motion.h1>
        <motion.p variants={line} custom={-1 * direction} className="mt-5 max-w-4xl will-change-transform text-xl leading-snug font-bold text-balance text-white/95 drop-shadow-[0_2px_12px_rgba(15,61,57,.38)] sm:text-2xl lg:text-3xl">
          {props.subheadline}
        </motion.p>
        ...
        <motion.div variants={actions} className="mt-7 flex flex-wrap items-center justify-center gap-4 will-change-transform">
          <Link className="group relative isolate min-w-44 overflow-hidden rounded-full bg-white px-7 py-3.5 text-xs font-black text-copad-deep shadow-[0_15px_35px_rgba(0,0,0,.16)] transition duration-500 hover:-translate-y-1 hover:text-white" href={`/${props.locale}/about`}>
          ...
          <Link className="group relative isolate min-w-44 overflow-hidden rounded-full border border-white/50 px-7 py-3.5 text-xs font-black text-white backdrop-blur-sm transition duration-500 hover:-translate-y-1 hover:border-copad-green" href={`/${props.locale}/therapeutic-areas`}>
