...
export function HomeHero(props: HomeHeroProps) {
  ...
  const titleOpacity = useTransform(smoothTimelineProgress, [0, 0.1, 1], [0.82, 1, 1]);
  const titleY = useTransform(smoothTimelineProgress, [0, 0.28, 1], [18, 0, -10]);
  const secondOpacity = useTransform(smoothTimelineProgress, [0, 0.14, 0.3, 1], [0.72, 0.86, 1, 1]);
  const secondY = useTransform(smoothTimelineProgress, [0, 0.3, 1], [26, 0, -4]);
  const buttonOpacity = useTransform(smoothTimelineProgress, [0.08, 0.24, 1], [0.55, 1, 1]);
  const buttonY = useTransform(smoothTimelineProgress, [0.08, 0.24, 1], [24, 0, 0]);
  ...
  return (
    <section ref={sectionRef} style={homeScrollSceneStyle(2)} onPointerMove={handlePointerMove} onPointerLeave={resetPointerLight} id="home" className="relative min-h-svh scroll-mt-20 bg-copad-deep text-white lg:h-[var(--scroll-scene-height)]">
      <div className="relative flex min-h-svh items-center overflow-hidden px-4 pt-24 pb-14 text-center sm:px-8 sm:pb-10 lg:sticky lg:top-0 lg:h-screen lg:px-12">
        ...
        <motion.div aria-hidden="true" className="pointer-events-none absolute inset-0 hidden origin-bottom bg-[linear-gradient(180deg,rgba(6,79,120,.2),rgba(6,79,120,.94))] lg:block" style={motionEnabled ? { y: mediaCurtainY } : { y: "-104%" }} />
        ...
        <motion.div aria-hidden="true" className="pointer-events-none absolute start-1/2 top-[46%] size-[44rem] -translate-x-1/2 -translate-y-1/2 rounded-full bg-[radial-gradient(circle,rgba(255,255,255,.14),rgba(0,163,196,.08)_34%,transparent_68%)] mix-blend-screen blur-2xl" style={motionEnabled ? { x: glowX, y: glowY } : undefined} />
        ...
        <motion.div className="relative mx-auto flex w-full max-w-5xl flex-col items-center lg:hidden" variants={mobileSequence} initial={reduceMotion ? false : "hidden"} animate="visible">
          ...
          <motion.p variants={mobileLine} custom={-1 * direction} className="mt-5 max-w-4xl text-lg leading-snug font-bold text-balance text-white/95 drop-shadow-[0_2px_12px_rgba(6,79,120,.38)] sm:text-2xl">{props.subheadline}</motion.p>
        ...
        <div className="relative z-10 mx-auto hidden w-full max-w-6xl lg:flex lg:flex-col lg:items-center lg:justify-center">
          <motion.p className="text-[10px] font-black tracking-[0.24em] text-copad-green uppercase" style={{ opacity: titleOpacity, y: titleY }}>{props.eyebrow}</motion.p>

          <motion.h1
            initial={reduceMotion ? false : { opacity: 0.72 }}
            animate={{ opacity: 1 }}
            transition={{ duration: 0.72, ease }}
            className={`mt-5 max-w-6xl px-6 font-display font-bold text-balance [backface-visibility:hidden] ${props.locale === "ar" ? "text-[clamp(3.7rem,5.4vw,5.5rem)] leading-[1.14] tracking-normal" : "text-[clamp(4rem,5.6vw,5.75rem)] leading-[.98] tracking-[-0.03em]"}`}
            style={{ y: titleY }}
          >
            {props.title}
          </motion.h1>

          <motion.p className="mt-6 max-w-4xl text-2xl leading-[1.18] font-bold text-balance text-white/94 drop-shadow-[0_4px_22px_rgba(6,79,120,.45)] xl:text-3xl" style={{ opacity: secondOpacity, y: secondY }}>
            {props.subheadline}
          </motion.p>

          <motion.div className="mt-8 flex items-center justify-center gap-4" style={{ opacity: buttonOpacity, y: buttonY }}>
            <HeroButtons primaryHref={primaryHref} secondaryHref={secondaryHref} primary={props.primaryCta} secondary={props.secondaryCta} cursorLabel={ui.interactionLabels.go} />
          </motion.div>
        </div>
        ...
        <motion.div aria-hidden="true" className="pointer-events-none absolute inset-x-[6%] bottom-0 h-[3px] origin-left bg-linear-to-r from-transparent via-copad-green to-transparent shadow-[0_0_24px_rgba(0,163,196,.65)] rtl:origin-right" style={motionEnabled ? { scaleX: handoffScale } : undefined} />
      </div>
    </section>
  );
}
