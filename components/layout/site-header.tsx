...
export function SiteHeader({ locale, transparent = false }: { locale: Locale; transparent?: boolean }) {
  ...
  return (
    <header className={`fixed inset-x-0 top-0 z-50 border-b transition-[box-shadow,backdrop-filter] duration-500 ${overlay ? "copad-scroll-header text-white backdrop-blur-md" : "border-copad-deep/10 bg-copad-white/92 text-copad-deep shadow-[0_12px_40px_rgba(1,61,96,.08)] backdrop-blur-xl"}`}>
      <div className="relative mx-auto flex h-[4.5rem] w-full max-w-[1440px] items-center px-4 sm:h-20 sm:px-8 lg:px-12">
        ...
        <div className="ms-auto hidden items-center gap-3 xl:flex">
          ...
          <Link href={`/${locale}/contact`} className="group relative isolate min-w-32 overflow-hidden rounded-full bg-copad-green px-6 py-3 text-center text-[11px] font-black text-white shadow-[0_12px_28px_rgba(0,144,175,.2)] transition duration-500 hover:-translate-y-0.5 hover:shadow-[0_18px_38px_rgba(0,144,175,.34)]">
          ...
          >
            ...
            <div className="relative z-10 min-h-0 flex-1 px-4 sm:px-8">
              ...
              >
                ...
                <Link onClick={() => setOpen(false)} href={`/${locale}/contact`} className="group relative isolate flex min-h-12 flex-1 items-center justify-center overflow-hidden rounded-full bg-copad-green px-5 text-xs font-black text-white shadow-[0_14px_34px_rgba(0,144,175,.25)] transition active:scale-[.98]">
